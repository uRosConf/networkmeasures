...
In addition and building on the concept of efficiency, network vulnerability as decribed by Latora et al (2005) is implemented with the `vulnerability` function: In a vulnearability anlysis, the goal is to investigate the impact of *attacks* or *improvements* of infrastructure components on the performance of the overall network. *Efficiency* is used as the metric for network performance. A vulnarability score each node $i$ in $G$, is computed as:

$$V(i)=1-\frac{E(G_i)}{E(G)}, ~where  ~ i \notin G_i$$
What if the `Altstadt-Loebenicht` became impassable? The resulting network $G_i$ would be
```{r}
g_i <- delete_edges(Koenigsberg, incident_edges(Koenigsberg, 1)[[1]])
plot(g_i) # see comments in vulnerability.. really skews the E score
g_i_better <- delete_vertices(Koenigsberg, 1)
plot(g_i_better)
```
...
Mathematically, a destruction of the latter two would "improve" travelling in the city, as the remaining system of quarters and bridges would be a complete graph.

Using `vulnerability_edges`, we can make the same investigation for the edges in the graph:
```{r}
vulnerability_edges(Koenigsberg)
```
Where there are more than one bridge connecting the quarters, vulnerability is $0$.
